			...
			<div class="mid">
				...
				<br>
					...
					<col width="25px"/><!-- 체크 -->
						...
						<td>${Storage_Top.OB_Num}</td>
							<td>
							<p style="border-bottom: 1px solid #d3caca;">${Storage_Top.OB_Name}</p>
							
							<span>${Storage_Top.OB_Kind}</span>/<span>${Storage_Top.OB_Form}</span>
							</td>
							
							<td>
							<p>주소 : ${Storage_Top.OB_S_Address}</p>
						
							</td>
							<td>${Storage_Top.OB_Floor}</td>
							<td>${Storage_Top.OB_Ho}</td>
							<td>${Storage_Top.OB_Pyeong}</td>
							<td>${Storage_Top.OB_Month_Cost}</td>
							<td>${Storage_Top.OB_Deposit}</td>
							<td>${Storage_Top.OB_Premium_Cost}</td>
							<td>${Storage_Top.OB_Maintenance_Cost}</td>
							<td>${Storage_Top.OB_Dealing_Cost}</td>
				...
				<br>
					...
					<col width="25px"/><!-- 체크 -->
						...
						<c:if test="${Storage_Mid.OB_State eq '계약'}">
						<tr style="background: #ffff00;">
						</c:if>
						<c:if test="${Storage_Mid.OB_State eq '방문'}">
						<tr style="background: #dbeef4;">
						</c:if>
						<c:if test="${Storage_Mid.OB_State eq '보류'}">
						<tr style="background: #f3dbdb;">
						</c:if>
						<c:if test="${Storage_Mid.OB_State eq '0'}">
						<tr style="background: white;">
						</c:if>
							<td>${Storage_Mid.OB_Num}</td>
							<td>
							<p style="border-bottom: 1px solid gray;">${Storage_Mid.OB_Name}</p>
							
							<span>${Storage_Mid.OB_Kind}</span>/<span>${Storage_Mid.OB_Form}</span>
							</td>
							
							<td>
							<p>주소 : ${Storage_Mid.OB_S_Address}</p>
						
							</td>
							<td>${Storage_Mid.OB_Floor}</td>
							<td>${Storage_Mid.OB_Ho}</td>
							<td>${Storage_Mid.OB_Pyeong}</td>
							<td>${Storage_Mid.OB_Month_Cost}</td>
							<td>${Storage_Mid.OB_Deposit}</td>
							<td>${Storage_Mid.OB_Premium_Cost}</td>
							<td>${Storage_Mid.OB_Maintenance_Cost}</td>
							<td>${Storage_Mid.OB_Dealing_Cost}</td>
							<td><a href="delete_storage?del_num=${Storage_Mid.OB_Num}&S_Title=${title_Num2}"><img src="resources/img/r.JPG" style="width: 25px; height: 30px;"></a></td>
